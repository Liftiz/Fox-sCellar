...
$wishListVins = $bdd->query('SELECT * FROM wishlist WHERE idPersonne=' . $idPersonne)->fetchAll(PDO::FETCH_ASSOC);
$insertNewVin = true;

// On verifie si la valeur de $idVin est null, si oui on passe insertNewVin a false
// pour dire que on ne va pas inserer de vin
if (is_null($idVin)) $insertNewVin = false;
foreach ($wishListVins as $wish) {
  if ($insertNewVin) {
    //var_dump($wish);
    if ((int) $wish["idVins"] == $idVin) $insertNewVin = false;
  }
}


if ($insertNewVin) {
  $bdd->prepare("INSERT INTO wishlist(idPersonne, idVins) VALUES(?, ?)")->execute(array($idPersonne, $idVin));
}

// Avant d'afficher tu dois sauvegarder le vin dans la table wishlist non
$wishlist =  $bdd->query('SELECT * FROM wishlist WHERE idPersonne=' . $idPersonne);
?>
...
<head>
  ...
  <title>Fox's Cellar: Ma Wishlist</title>
</head>
<body>
  ...
  <div class="flip-card">
    ...
    <div class="card-back">
      ...
      <button class="supp"><a href="delete.php?idWish=<?php echo $donnees['idWish'] ?>">Supprimer</a></button>
